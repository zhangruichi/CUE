...
class Trainer:
    ...
    def build_cue(self):
        self.llm_fn = get_multi_fn(self.classnames, self.cfg.json_path,self.device)
        self.vlm_fn = vlm_multi_fn
        self.bla_loss = get_multi_bla_loss(self.cls_num_list)
    ...
    def build_data_loader(self):
        ...
        self.train_loader = DataLoader(train_dataset, batch_size=cfg.micro_batch_size, shuffle=True, num_workers=cfg.num_workers, pin_memory=True)

        self.train_init_loader = DataLoader(train_init_dataset, batch_size=64, sampler=init_sampler, shuffle=False, num_workers=cfg.num_workers, pin_memory=True)

        self.train_test_loader = DataLoader(train_test_dataset, batch_size=64, shuffle=False, num_workers=cfg.num_workers, pin_memory=True)

        self.test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False, num_workers=cfg.num_workers, pin_memory=True)
    ...
    def build_model(self):
        ...
        print(f"Loading ZS_CLIP (backbone: {cfg.backbone})")
        clip_model = load_clip_to_cpu(cfg.backbone, cfg.prec)
        self.zs_model = ZeroShotCLIP(clip_model)
        self.zs_model.to(self.device)
        template = "a photo of a {}."
        prompts = self.get_tokenized_prompts(classnames, template)
        self.zs_model.init_text_features(prompts)
        self.zs_model.eval()
        
        # add this to fix all seed avoid zs_init influence the result
    ...
    def build_optimizer(self):
        ...
        self.optim = torch.optim.SGD([{"params": self.tuner.parameters()}, {"params": self.head.parameters()}], lr=cfg.lr, weight_decay=cfg.weight_decay, momentum=cfg.momentum)
    ...
    def train(self):
        ...
        print("Note that the printed training acc is not precise.", "To get precise training acc, use option ``test_train True``.")
